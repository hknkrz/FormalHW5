...
TEST(Minimize, Default) {
  Authomat* y = Authomat::Build("ab.ca.+*a.c*.b.");
  y->DelEps();
  y = y->Determine();
  y->FullAuth();
  y = y->Minimize();
  ASSERT_EQ(y->GetConds()->size(), 6);
  ASSERT_EQ(y->GetDelta()->size(), 18);
}

TEST(Minimize, MidExp) {
  Authomat* y = Authomat::Build("ab.cbcb..*.a.+a*.");
  y->DelEps();
  y = y->Determine();
  y->FullAuth();
  y = y->Minimize();
  ASSERT_EQ(y->GetConds()->size(), 7);
  ASSERT_EQ(y->GetDelta()->size(), 21);
}
TEST(Minimize, LongExp) {
  Authomat* y = Authomat::Build("cab..acb..*.bba..ccab...*.+");
  y->DelEps();
  y = y->Determine();
  y->FullAuth();
  y = y->Minimize();
  ASSERT_EQ(y->GetConds()->size(), 12);
  ASSERT_EQ(y->GetDelta()->size(), 36);
}
